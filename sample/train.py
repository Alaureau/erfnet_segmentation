...
aug_func = create_augmentation_func_for_segmentation(
    shadow=(0.01, 0.8),
    shadow_file="../images/shadow_pattern.jpg",
...
with tf.device('/device:GPU:0'):
    if __name__ == '__main__':
        ...
        data_file = "../pickle_jar/data_HD4.pickle"
        ...
        model_name = "MON24"
        model = SegmentationModel(model_name, img_shape=[424,240], n_classes=len(data["id2label"]), l2=2e-4)
        ...
        # TRAIN
        print("ALPHA --- 8e-4")
        model.train(data, n_epochs=40, print_every=5, batch_size=6, alpha=8e-4, dropout=0.3, l2=2e-4, aug_func=aug_func, viz_every=1)

        print("ALPHA --- 5e-4")
        model.train(data, n_epochs=20, print_every=5, batch_size=6, alpha=5e-4, dropout=0.3, l2=2e-4, aug_func=aug_func, viz_every=1)
        print("ALPHA --- 2e-4")
        model.train(data, n_epochs=20, print_every=5, batch_size=6, alpha=2e-4, dropout=0.3, l2=2e-4, aug_func=aug_func, viz_every=1)
        print("ALPHA --- 1e-4")
        model.train(data, n_epochs=20, print_every=5, batch_size=6, alpha=1e-4, dropout=0.3, l2=2e-4, aug_func=aug_func, viz_every=1)
        print("ALPHA --- 5e-5")
        model.train(data, n_epochs=20, print_every=5, batch_size=6, alpha=5e-5, dropout=0.3, l2=2e-4, aug_func=aug_func, viz_every=1)

        '''print("ALPHA --- 5e-4")
        model.train(data, n_epochs=80, print_every=5, batch_size=8, alpha=5e-4, dropout=0.3, l2=2e-4, aug_func=aug_func, viz_every=1)
        print("ALPHA --- 2e-4")
